...
function applyConfig() {
    if (typeof CONFIG !== 'undefined') {
        ...
        if (typeof MediaUtils !== 'undefined') {
            ...
            // 初始化关于我们区域背景媒体（添加这部分）
            MediaUtils.initSectionBackground('#about', CONFIG.backgrounds.aboutBackground);
            console.log('关于我们区域背景媒体已初始化');
        } else {
            ...
            // 传统方式设置关于我们背景（添加这部分）
            setTraditionalBackground('#about', CONFIG.backgrounds.aboutBackground);
        }
...
function applyDynamicStyles() {
    ...
    const cssRules = `
        ...
        /* 区域背景 */
        /* 移除#about的背景色设置，以允许背景图片显示 */
        /* #about {
            background-color: ${CONFIG.colors.sections.about};
        } */
...
function updateDemoVideoCarousel() {
    ...
    if (cards.length === 1) {
        ...
        if (currentVideo && currentVideo.video) {
            // 检查内容是图片还是视频
            const isImage = currentVideo.video.endsWith('.jpg') || 
                           currentVideo.video.endsWith('.jpeg') || 
                           currentVideo.video.endsWith('.png') || 
                           currentVideo.video.endsWith('.gif') || 
                           currentVideo.video.endsWith('.webp') || 
                           currentVideo.video.endsWith('.avif');
            
            const container = card.querySelector('.org-image');
            ...
            if (isImage) {
                ...
                img.src = currentVideo.video;
                img.alt = currentVideo.name;
                img.style.cssText = `
                    width: 100%;
                    height: 100%;
                    object-fit: cover;
                ...
                console.log(`设置演示图片 src: ${currentVideo.video}`);
            } else {
                // 如果是视频，创建iframe
        ...
        cards.forEach((card, idx) => {
            ...
            if (idx === currentDemoVideoIndex && demoVideos[idx]) {
                // 检查内容是图片还是视频
                const isImage = demoVideos[idx].video && (
                    demoVideos[idx].video.endsWith('.jpg') || 
                    demoVideos[idx].video.endsWith('.jpeg') || 
                    demoVideos[idx].video.endsWith('.png') || 
                    demoVideos[idx].video.endsWith('.gif') || 
                    demoVideos[idx].video.endsWith('.webp') || 
                    demoVideos[idx].video.endsWith('.avif')
                );
                ...
                if (isImage && demoVideos[idx].video) {
                    // 如果是图片，创建img标签
                    const img = document.createElement('img');
                    img.src = demoVideos[idx].video;
                    img.alt = demoVideos[idx].name;
                    img.style.cssText = `
                        width: 100%;
                        height: 100%;
                        object-fit: cover;
...
function updateOrgCarousel() {
    ...
    if (cards.length === 1) {
        ...
        if (currentOrg) {
            ...
            if (stats) {
                stats.innerHTML = `
                    <span>${currentOrg.host}</span>
                    <span>${currentOrg.time}</span>
        ...
        if (about.content.text2) {
            ...
            if (aboutText2) {
                ...
                about.content.text2.forEach(text => {
                    const p = document.createElement('p');
                    // 关键点：将textContent改为innerHTML，这样\n会被解析为换行
    ...
    if (footer.sections.follow) {
        ...
        if (socialLinksContainer && footer.sections.follow.socialLinks) {
            ...
            footer.sections.follow.socialLinks.forEach(link => {
                ...
                // 构建显示文本，支持任意数量的name属性
                let displayText = '';
                let index = 1;
                let currentName = link.name;
                
                // 循环获取所有name属性，直到没有新的name属性
